...
#SBATCH -o /scratch/msarava7/IAI/Results/Benchmarking_Runs/slurm.brats.seg.mednext.%x.%j.out # file to save job's STDOUT (%j = JobId)
#SBATCH -e /scratch/msarava7/IAI/Results/Benchmarking_Runs/slurm.brats.seg.mednext.%x.%j.err # file to save job's STDERR (%j = JobId)
...
mednextv1_train 3d_fullres nnUNetTrainerV2_MedNeXt_B_kernel3 Task137_BraTS2021 2 -p nnUNetPlansv2.1_trgSp_1x1x1 -c


# E-mail diagnostic results to yourself using mailserver and a heredoc
mail -s "MedNeXt BraTS Fold 2 MICCAI 2023" ${USER}@asu.edu << EOF
Hi,
