...
#SBATCH -o /scratch/msarava7/IAI/Results/Benchmarking_Runs/slurm.btcv.seg.swinunetr.%x.%j.out # file to save job's STDOUT (%j = JobId)
#SBATCH -e /scratch/msarava7/IAI/Results/Benchmarking_Runs/slurm.btcv.seg.swinunetr.%x.%j.err # file to save job's STDERR (%j = JobId)
...
mednextv1_train 3d_fullres nnUNetTrainerV2_SwinUNETR_128x128x128 Task017_AbdominalOrganSegmentation 0 -p nnUNetPlansv2.1_trgSp_1x1x1 -val


# E-mail diagnostic results to yourself using mailserver and a heredoc
mail -s "SwinUNETR BTCV Fold 0 MICCAI 2023" ${USER}@asu.edu << EOF
Hi,
